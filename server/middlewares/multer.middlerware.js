...
export const processAndUploadImage = async (file) => {
  ...
  }/images/${Date.now()}-${originalname.toLowerCase().split(" ").join("-")}`;
...
export const uploadPDF = async (file) => {
  ...
  const filename = `${
    process.env.UPLOAD_FOLDER
  }/pdfs/${Date.now()}-${originalname.toLowerCase().split(" ").join("-")}`;
...
export const uploadAudio = async (file) => {
  ...
  const filename = `${
    process.env.UPLOAD_FOLDER
  }/audio/${Date.now()}-${originalname.toLowerCase().split(" ").join("-")}`;
