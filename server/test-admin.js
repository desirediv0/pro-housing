...
async function testAdminSystem() {
  ...
  try {
    ...
    const propertiesResponse = await axios.get(`${API_BASE}/properties/all`, {
      headers,
    });
    console.log(
      "✅ Properties retrieved:",
      propertiesResponse.data.data.properties.length,
      "properties found"
    );
...
if (
  process.argv[1] === new URL(import.meta.url).pathname.replace(/^\/[A-Z]:/, "")
) {
  testAdminSystem();
}
